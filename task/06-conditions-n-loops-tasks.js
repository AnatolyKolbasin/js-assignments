...
function doRectanglesOverlap(rect1, rect2) {
  return ((rect1.width + rect2.width)/2 > Math.abs(rect1.left - rect2.left) &&
        (rect1.height + rect2.height)/2 > Math.abs(rect1.top - rect2.top))
}
...
function isInsideCircle(circle, point) {
    return Math.sqrt((point.x-circle.center.x)*(point.x-circle.center.x) + (point.y-circle.center.y)*(point.y-circle.center.x)) < circle.radius;
}
